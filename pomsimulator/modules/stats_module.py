...
def load_array(path_npz):
    '''Loads a NPZ file containing speciation information'''
    SuperArr = np.load(path_npz)["SupArray"]
    IndexArr = np.load(path_npz)["IndexArray"]
    pHArr = np.load(path_npz)["pH"]
    return SuperArr,IndexArr,pHArr

def mask_models(SuperArr,speciation_labels,threshold=1.1,C=0.1,idx=0):
    ...
    '''
    v_ctt = np.array([Lab_to_stoich(lab)[idx] for lab in speciation_labels])
...
def get_features(model,pH,v_ctt):
    ...
    Returns:
        ...
        in the system.
    '''
    indices = get_pka(model)
    pk_vals = pH[indices]
    width = np.abs(pk_vals[1::2] - pk_vals[0::2])
    pos = pk_vals[0::2] + width*0.5
...
def get_cluster_members(km_obj,verbose=False):
